...
const sages = {
    confucius: {
        ...
        responses: [
            "반갑습니다. 논어에서 이르기를 **'학이시습지 불역열호(學而時習之 不亦說乎)'**라 하였습니다. 배우고 때로 익히니 또한 즐겁지 아니한가라는 이 말씀은 단순히 지식의 습득을 넘어, 당신의 고민조차 삶의 소중한 배움으로 삼으라는 뜻입니다. 당신의 고통은 현재 당신이 군자로 나아가기 위한 수양의 과정에 있음을 보여주는 증거입니다. 먼저 자신의 마음을 정직하게 들여다보고, 내가 서고자 할 때 남을 먼저 세워주는 **'기욕립이립인(己欲立而立人)'**의 마음을 가져보십시오. 타인과의 관계에서 오는 갈등은 결국 예(禮)를 통해 다스려질 수 있습니다. 조급해하지 말고 매일 조금씩 자신의 덕을 쌓아간다면, 머지않아 평안을 찾게 될 것입니다. 당신의 진실된 노력을 하늘은 결코 외면하지 않을 것입니다.",
...
function addMessage(type, text, sage = null) {
    ...
    let content = '';
    if (type === 'user') {
        content = `<div class="bg-amber-900 text-white rounded-2xl px-6 py-3 max-w-[85%] shadow-lg border border-amber-800">${text}</div>`;
    } else if (type === 'system') {
        content = `<div class="bg-stone-200 text-stone-600 text-xs font-bold rounded-full px-6 py-2 mx-auto tracking-widest uppercase">${text}</div>`;
    } else {
        content = `
            <div class="flex flex-col gap-2 max-w-[85%]">
                <span class="text-xs font-extrabold text-amber-900 ml-2 serif uppercase tracking-tighter">${sage.title}</span>
                <div class="bg-white border-2 border-stone-200 rounded-3xl px-6 py-4 shadow-xl text-stone-800 leading-relaxed italic">
